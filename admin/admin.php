...
<script type="text/javascript">
	jQuery( function( $ ) {
		...
		<?php foreach ( $post_types as $post_type_obj ) : ?>
			$( '#form_<?php echo esc_attr( $post_type_obj->name ); ?>' ).submit( function() {
				var total_check_num = $( '#form_<?php echo esc_attr( $post_type_obj->name ); ?> input.post_status:checked' ).length;
				if ( total_check_num === 0 ) {
					alert( '<?php $this->e( '"Status" is a required field.', '"ステータス"は必須項目です' ); ?>' );
					return false;
				}
				if ( ! $( '#form_<?php echo esc_attr( $post_type_obj->name ); ?> input.limit' ).val().match( /^[0-9]+$/ ) ) {
			...
			// offset の表示切替
			$( '#form_<?php echo esc_attr( $post_type_obj->name ); ?> input.limit' ).on( 'change', function() {
				var target = $( this ).attr( 'data-target' );
				if ( $( this ).val() > 0 ) {
					$( target ).fadeIn();
				} else {
...
<div class="wrap plugin-wrap">
	...
	<div class="plugin-main-area">
		...
		<?php if ( ! is_writable( $filename ) ) : // phpcs:ignore WordPress.WP.AlternativeFunctions ?>
			<div class="error">
				<p>
		...
		<ul class="plugin_tab">
			<?php foreach ( $post_types as $post_type_obj ) : ?>
				<li class="plugin_tab-<?php echo esc_attr( $post_type_obj->name ); ?>"><?php echo esc_html( $post_type_obj->labels->name ); ?></li>
			<?php endforeach; ?>
		</ul>

		<div class="plugin_contents">
			<?php foreach ( $post_types as $post_type_obj ) : ?>
				<div class="plugin_content js-csv-content" data-post-type="<?php echo esc_attr( $post_type_obj->name ); ?>">
					<form action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="post" id="form_<?php echo esc_attr( $post_type_obj->name ); ?>" target="_blank">
						<input type="hidden" name="action" value="download">
						<?php wp_nonce_field( 'csv_exporter' ); ?>

						<div class="tool-box">
							<h3><?php echo esc_html( $post_type_obj->labels->name ); ?> <?php $this->e( 'Settings', '設定' ); ?></h3>
							<ul class="setting_list">
								<li><label><input type="radio" name="post_id" value="post_id" checked="checked" required>*<?php $this->e( 'Post ID', '投稿ID' ); ?></label></li>
								<li><label><input type="radio" name="type" value="<?php echo esc_attr( $post_type_obj->name ); ?>" checked="checked" required>*<?php $this->e( 'Post Type', '投稿タイプ' ); ?></label></li>
						...
						<div class="tool-box">
							<h3><?php $this->e( 'Taxonomies', 'タクソノミー' ); ?>
								<span class="all_checks">
									[
									<a href="javascript:void(0);" class="all_checked" data-target="#form_<?php echo esc_attr( $post_type_obj->name ); ?> .cf_taxonomy"><?php $this->e( 'Select all', '全選択' ); ?></a>
									<a href="javascript:void(0);" class="all_checkout" data-target="#form_<?php echo esc_attr( $post_type_obj->name ); ?> .cf_taxonomy"><?php $this->e( 'Unselect all', '全解除' ); ?></a>
									]
								</span>
							</h3>
							<ul class="setting_list">
								...
								foreach ( $post_taxonomies as $post_taxonomy ) :
									if ( ! is_object_in_taxonomy( $post_type_obj->name, $post_taxonomy->name ) ) {
										continue;
									}
									?>
						...
						<div class="tool-box">
							<h3><?php $this->e( 'Custom Fields', 'カスタムフィールド' ); ?>
								<span class="all_checks">
									[
									<a href="javascript:void(0);" class="all_checked" data-target="#form_<?php echo esc_attr( $post_type_obj->name ); ?> .cf_checkbox"><?php $this->e( 'Select all', '全選択' ); ?></a>
									<a href="javascript:void(0);" class="all_checkout" data-target="#form_<?php echo esc_attr( $post_type_obj->name ); ?> .cf_checkbox"><?php $this->e( 'Unselect all', '全解除' ); ?></a>
									]
								</span>
							</h3>
							<?php $cf_results = $this->get_custom_field_list( $post_type_obj->name ); ?>
							<ul class="setting_list">
								<?php if ( ! empty( $cf_results ) ) : ?>
									<?php foreach ( $cf_results as $cf_field ) : ?>
										<li><label><input type="checkbox" name="cf_fields[]" class="cf_checkbox" checked="checked" value="<?php echo esc_attr( $cf_field['meta_key'] ); ?>"> <?php echo esc_html( $cf_field['meta_key'] ); ?></label></li>
									<?php endforeach; ?>
								<?php else : ?>
							...
							<table class="setting_table">
								<tbody>
									<tr>
										...
										<td>
											<input type="number" name="limit" class="limit" value="0" min="0" data-target=".offset-<?php echo esc_attr( $post_type_obj->name ); ?>"> <?php $this->e( '*All downloaded if "0" selected.', '※0の場合はすべてダウンロード' ); ?>
											<div class="offset offset-<?php echo esc_attr( $post_type_obj->name ); ?>" style="display:none;">
						...
						<?php $submit_disabled = ! is_writable( $filename ) ? 'disabled' : ''; // phpcs:ignore WordPress.WP.AlternativeFunctions ?>
						<p class="submit"><input type="submit" class="button-primary" value="<?php $this->e( 'Export', 'エクスポート' ); ?> <?php echo esc_html( $post_type_obj->labels->name ); ?> CSV" <?php echo esc_attr( $submit_disabled ); ?>></p>
					</form>
				</div>
			<?php endforeach; ?>
		</div>
